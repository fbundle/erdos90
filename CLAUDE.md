...
| `Erdos90/Mathlib4_Extra/ClassFieldTheory.lean` | Phase E14+E15 — `HilbertClassFieldExt K` structure with Galois + Artin reciprocity (`isGaloisHK`, `isAbelianGaloisHK`, `artinReciprocity : ClassGroup (𝓞 K) ≃* Gal(H/K)`) + `hilbertClassField_exists` postulate (sorry).  PROVED corollaries: `rootDiscr_hcf_eq`, `card_gal_hcf_eq_classNumber`, `finiteDimensional`, `isTotallyComplex`, `bijective_algebraMap_of_classNumber_one`.  POSTULATES (labelled sorries): `isCMField_postulate` (CM preservation), `hilbert_principal_ideal_postulate` (Hilbert 94). |
| `Erdos90/Mathlib4_Extra/RayClassField.lean` | Phase E15 (2026-05-27) — `MaxProPExt K p S` (HMR's `K_S^{(p)}`, max pro-`p` extension unramified outside `S`) + `RayClassField K 𝔪` structures with `maxProPExt_exists` postulate (sorry).  Documents WHY HMR uses S-restricted (tame) ramification: relaxing constraint allows infinite towers with `rd < 84`. |
| `Erdos90/Mathlib4_Extra/Chebotarev.lean` | Phase E14 — Chebotarev density + Ihara's split-primes labelled postulates (off-path).  Documents WHY `chebotarev_fixed_Q` doesn't decompose into "Chebotarev + split persistence": unramified extensions preserve `ramificationIdx = 1` but not `inertiaDeg = 1`. |
